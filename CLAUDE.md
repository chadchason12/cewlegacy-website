...
**Gotcha if `buyer_leads` (or any future table) is ever recreated:** an RLS policy alone isn't enough — Postgres also requires an explicit `GRANT` to the `anon`/`authenticated` roles, which Supabase's own table editor adds automatically but a raw SQL migration does not. Forgetting this makes every insert fail with `permission denied for table ...` even though the policy looks correct. This exact bug happened once (Sep 15, 2026) and was fixed with `grant insert on public.buyer_leads to anon; grant select, update on public.buyer_leads to authenticated; grant select, insert, update, delete on public.buyer_leads to service_role;`.
